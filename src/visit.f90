...
SUBROUTINE visit
  ...
  DO tile = 1, tiles_per_chunk
    IF(chunk%task.EQ.parallel%task) THEN
      ...
      WRITE(u,'(a)')'ASCII'
      WRITE(u,'(a)')'target dataSET RECTILINEAR_GRID'
      ...
      WRITE(u,'(a)')'0'
      WRITE(u,'(a,i20)')'CELL_target data ',nxc*nyc
      ...
      DO k=chunk%tiles(tile)%t_ymin,chunk%tiles(tile)%t_ymax
        ...
        ENDDO
      ENDDO
      WRITE(u,'(a,i20)')'POINT_target data ',nxv*nyv
...
!$omp target update from(density0)
!$omp target update from(energy0)
!$omp target update from(pressure)
!$omp target update from(viscosity)
!$omp target update from(xvel0)
!$omp target update from(yvel0)
!$omp target update from(vertexx)
!$omp target update from(vertexy)
